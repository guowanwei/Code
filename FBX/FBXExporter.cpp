...
bool FBXExporter::LoadModelInfo(const char* ModelFileName, CommonMeshData& meshData)
{
	...
	if (!fbxImporter->Initialize(ModelFileName, -1, mFBXManager->GetIOSettings()))
	{
		return false;
	}
	...
	{
		BaseCommonVertex vertex;
		vertex.mPosition = mVertices[i].mPosition;
		vertex.mNormal = mVertices[i].mNormal;
		vertex.mTangent = mVertices[i].mTangent;
		vertex.mBinormal = mVertices[i].mBinormal;
		vertex.mUV = mVertices[i].mUV;
		meshData.vertices.push_back(vertex);
	}
	for (int i = 0; i < mTriangles.size(); ++i)
	{
		if (meshData.materials.find(mTriangles[i].mMaterialIndex) == meshData.materials.end())
		{
			meshData.materials[mTriangles[i].mMaterialIndex].mDiffuseMapName = mMaterialLookUp[mTriangles[i].mMaterialIndex]->mDiffuseMapName;
			meshData.materials[mTriangles[i].mMaterialIndex].mNormalMapName = mMaterialLookUp[mTriangles[i].mMaterialIndex]->mNormalMapName;
			meshData.materials[mTriangles[i].mMaterialIndex].RoughnessMetallicMapName = mMaterialLookUp[mTriangles[i].mMaterialIndex]->mGlossMapName;
			meshData.materials[mTriangles[i].mMaterialIndex].startIndex = 3 * i;
			meshData.materials[mTriangles[i].mMaterialIndex].startIndex = 3 * i;
			meshData.materials[mTriangles[i].mMaterialIndex].countIndex = 3;
		}
		else
		{
			meshData.materials[mTriangles[i].mMaterialIndex].countIndex = 3 * i + 3 - meshData.materials[mTriangles[i].mMaterialIndex].startIndex;
		}
		for (int j = 0; j < mTriangles[i].mIndices.size(); ++j)
		{
			meshData.indices.push_back(mTriangles[i].mIndices[j]);
		}
	}
	//处理材质texture路径
	for (std::map<UINT, MaterialDesc>::iterator iter = meshData.materials.begin(); iter != meshData.materials.end(); ++iter)
	{
		int pos = iter->second.mDiffuseMapName.find_last_of('\\');
		iter->second.mDiffuseMapName = std::string("Resource\\Texture\\dds\\")+ iter->second.mDiffuseMapName.substr(pos+1, iter->second.mDiffuseMapName.size() - pos-4)+ std::string("dds");

		pos = iter->second.mNormalMapName.find_last_of('\\');
		iter->second.mNormalMapName = std::string("Resource\\Texture\\dds\\") + iter->second.mNormalMapName.substr(pos + 1, iter->second.mNormalMapName.size() - pos-4) + std::string("dds");

		pos = iter->second.RoughnessMetallicMapName.find_last_of('\\');
		iter->second.RoughnessMetallicMapName = std::string("Resource\\Texture\\dds\\") + iter->second.RoughnessMetallicMapName.substr(pos + 1, iter->second.RoughnessMetallicMapName.size() - pos - 4) + std::string("dds");
	}
	return true;
}
...
void FBXExporter::ProcessMesh(FbxNode* inNode)
{
	...
	for (unsigned int i = 0; i < mTriangleCount; ++i)
	{
		...
		for (unsigned int j = 0; j < 3; ++j)
		{
			...
			//temp.mUV.x = 1 - temp.mUV.x;
			//哈哈原来在这里,
			temp.mUV.y = 1 - temp.mUV.y;
...
void FBXExporter::Optimize()
{
	...
	*/
	std::map<PNTIWVertex,int> tmpUniqueVertices;
	for (unsigned int i = 0; i < mTriangles.size(); ++i)
	{
		for (unsigned int j = 0; j < 3; ++j)
		{
			// If current vertex has not been added to
			// our unique vertex list, then we add it
			if (tmpUniqueVertices.find(mVertices[i * 3 + j]) == tmpUniqueVertices.end())
			{
				tmpUniqueVertices[mVertices[i * 3 + j]] = 1;
			}
		}
	}
	std::vector<PNTIWVertex> uniqueVertices;
	for (std::map<PNTIWVertex, int>::iterator iter = tmpUniqueVertices.begin(); iter != tmpUniqueVertices.end(); ++iter)
	{
		uniqueVertices.push_back(iter->first);
	}
	...
	for(unsigned int i = 0; i < mTriangles.size(); ++i)
	{
		for(unsigned int j = 0; j < 3; ++j)
		{
			...
			int size = sizeof(PNTIWVertex);
			std::vector<PNTIWVertex>::iterator find = std::lower_bound(uniqueVertices.begin(), uniqueVertices.end(), mVertices[i * 3 + j]);
			if (find != uniqueVertices.end())
				mTriangles[i].mIndices[j] = find - uniqueVertices.begin();
			else
				mTriangles[i].mIndices[j] = 0;
				...
				else
				{
					...
					if(texture)
					{
						...
						if(fileTexture)
						{
							...
							{
								ioMaterial->mGlossMapName = fileTexture->GetFileName();
							}
							else
							{
								int test = 0;
